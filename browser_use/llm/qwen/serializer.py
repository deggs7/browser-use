...
class QwenMessageSerializer:
	"""Serializer for converting browser-use messages to Qwen messages."""
	...
	def _serialize_content(content: Any) -> str | list[dict[str, Any]]:
		...
		for part in content:
			if part.type == 'text':
				serialized.append({'type': 'text', 'text': QwenMessageSerializer._serialize_text_part(part)})
			elif part.type == 'image_url':
				serialized.append(QwenMessageSerializer._serialize_image_part(part))
	...
	def serialize(message: BaseMessage) -> MessageDict:
		if isinstance(message, UserMessage):
			return {
				'role': 'user',
				'content': QwenMessageSerializer._serialize_content(message.content),
			}
		...
		if isinstance(message, AssistantMessage):
			...
			if message.tool_calls:
				msg['tool_calls'] = QwenMessageSerializer._serialize_tool_calls(message.tool_calls)
			return msg
	...
	def serialize_messages(messages: list[BaseMessage]) -> list[MessageDict]:
		return [QwenMessageSerializer.serialize(m) for m in messages]
